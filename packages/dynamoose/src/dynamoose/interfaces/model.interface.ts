...
import { Condition, ConditionFunction, ConditionInitializer } from 'dynamoose/dist/Condition';
...
export type Item<T> = {
  ...
  populate(settings: PopulateSettings, callback: CallbackType<Item<T>, any>): void;
...
export interface Model<Data, Key, DefaultFields extends keyof any = ''> {
  ...
  batchGet(keys: Key[], callback: CallbackType<ModelBatchGetItemsResponse<Item<Data>>, any>): void;
  batchGet(
    keys: Key[],
  ...
  batchPut(items: OptionalOmit<Data, DefaultFields>[]): Promise<UnprocessedItems<Item<Data>>>;
  batchPut(
  ...
  batchDelete(keys: Key[], callback: CallbackType<UnprocessedItems<Key>, any>): void;
  batchDelete(
    keys: Key[],
  ...
  create(item: OptionalOmit<Data, DefaultFields>, callback: CallbackType<Item<Data>, any>): void;
  create(
  ...
  serializeMany(itemsArray: Data[], nameOrOptions: SerializerOptions | string): ObjectType[];
}
...
export interface GetTransaction<Key> {
  ...
  (key: Key, settings: ModelGetSettings & { return: 'item' }): GetTransactionResult;
  (key: Key, settings: ModelGetSettings & { return: 'request' }): GetTransactionResult;
}
export interface CreateTransaction<Data> {
  ...
  (item: Partial<Data>, settings: ItemSaveSettings & { return: 'item' }): CreateTransactionResult;
...
export interface DeleteTransaction<Key> {
  ...
  (key: Key, settings: ModelDeleteSettings & { return: 'request' }): DeleteTransactionResult;
  (key: Key, settings: ModelDeleteSettings & { return: null }): DeleteTransactionResult;
