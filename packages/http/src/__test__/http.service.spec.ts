import { INestApplication } from '@nestjs/common';
import { Test, TestingModule } from '@nestjs/testing';
import { ConfigModule, ConfigService } from '@nestjs/config';

import { HttpClientService } from '../lib/services/http.service';
import { HttpClientModule } from '../lib/http.module';
import { config } from './utils';
import { LoggingService } from '@tresdoce-nestjs-toolkit/logging';

const API_NESTJS_STARTER = 'https://jsonplaceholder.typicode.com';

const mockRequestBody = {
  userId: 1,
  id: 1,
  title: 'sunt aut facere repellat provident occaecati excepturi optio reprehenderit',
  body: 'quia et suscipit\nsuscipit recusandae consequuntur expedita et cum\nreprehenderit molestiae ut ut quas totam\nnostrum rerum est autem sunt rem eveniet architecto',
};

const mockedManifest = {
  name: 'nestjs-logging',
  version: '1.0.0',
};

describe('HttpService', () => {
  let app: INestApplication;
  let httpClientService: HttpClientService;
  let loggingService: LoggingService;

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      imports: [
        ConfigModule.forRoot({
          isGlobal: true,
          load: [config],
        }),
        HttpClientModule.registerAsync({
          imports: [ConfigModule],
          useFactory: async (configService: ConfigService) =>
            configService.get('config.httpOptions'),
          inject: [ConfigService],
        }),
      ],
    }).compile();
    app = module.createNestApplication();
    await app.init;
    httpClientService = module.get<HttpClientService>(HttpClientService);
    loggingService = module.get<LoggingService>(LoggingService);
  });

  afterEach(async () => {
    await app.close();
  });

  it('should be defined', () => {
    expect(httpClientService).toBeDefined();
    expect(httpClientService).toBeInstanceOf(HttpClientService);
    expect(httpClientService.axiosRef).not.toBeNull();
  });

  it('should be return status 200 - request', async () => {
    loggingService.readFile = jest.fn().mockImplementation(() => mockedManifest);
    const { status, data } = await httpClientService.request({
      url: `${API_NESTJS_STARTER}/posts`,
      method: 'GET',
    });
    expect(status).toBe(200);
    expect(data.length).toBeGreaterThan(0);
  });

  it('should be return status 200 - get', async () => {
    loggingService.readFile = jest.fn().mockImplementation(() => mockedManifest);
    const { status, data } = await httpClientService.get(`${API_NESTJS_STARTER}/posts`);
    expect(status).toBe(200);
    expect(data.length).toBeGreaterThan(0);
  });

  it('should be return status 201 - post', async () => {
    loggingService.readFile = jest.fn().mockImplementation(() => mockedManifest);
    const { status, data } = await httpClientService.post(`${API_NESTJS_STARTER}/posts`, {
      data: mockRequestBody,
    });
    expect(status).toBe(201);
    expect(data).toEqual({
      ...mockRequestBody,
      id: 101,
    });
  });

  it('should be return status 200 - get by id', async () => {
    loggingService.readFile = jest.fn().mockImplementation(() => mockedManifest);
    const { status, data } = await httpClientService.get(`${API_NESTJS_STARTER}/posts/1`);
    expect(status).toBe(200);
    expect(data).toEqual(mockRequestBody);
  });

  it('should be return status 200 - put', async () => {
    loggingService.readFile = jest.fn().mockImplementation(() => mockedManifest);
    const { status, data } = await httpClientService.put(`${API_NESTJS_STARTER}/posts/1`, {
      data: {
        ...mockRequestBody,
        title: 'mockito',
      },
    });
    expect(status).toBe(200);
    expect(data).toEqual({ ...mockRequestBody, id: 1, title: 'mockito' });
  });

  it('should be return status 200 - delete', async () => {
    loggingService.readFile = jest.fn().mockImplementation(() => mockedManifest);
    const { status, data } = await httpClientService.delete(`${API_NESTJS_STARTER}/posts/2`);
    expect(status).toBe(200);
    expect(data).toEqual({});
  });

  it('should be return status 200 - patch', async () => {
    loggingService.readFile = jest.fn().mockImplementation(() => mockedManifest);
    const { status, data } = await httpClientService.patch(`${API_NESTJS_STARTER}/posts/1`, {
      data: {
        ...mockRequestBody,
        title: 'mockito',
      },
    });
    expect(status).toBe(200);
    expect(data).toEqual({ ...mockRequestBody, id: 1, title: 'mockito' });
  });

  it('should be return status 200 - head', async () => {
    loggingService.readFile = jest.fn().mockImplementation(() => mockedManifest);
    const { status, headers } = await httpClientService.head(`${API_NESTJS_STARTER}/posts`);
    expect(status).toBe(200);
    expect(headers).toHaveProperty('etag');
  });

  it('should be return error', async () => {
    loggingService.readFile = jest.fn().mockImplementation(() => mockedManifest);
    try {
      await httpClientService.get(`${API_NESTJS_STARTER}/postss`);
    } catch (error) {
      expect(error.statusCode).toBe(404);
    }
  });
});
